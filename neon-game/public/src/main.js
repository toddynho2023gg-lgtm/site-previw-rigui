...
function bindUI(){
  ...
  ui.el('settingsBtn').addEventListener('click',()=>{setPlayerInput(false);document.exitPointerLock?.()});ui.el('settingsClose').addEventListener('click',()=>{setPlayerInput(true);requestGamePointer()});
...
function bindNet(){
  ...
  net.on('blackjack_state',m=>{setPlayerInput(false);ui.blackjackState(m.state)});net.on('blackjack_result',m=>{ui.blackjackResult(m.result);m.result.payout>m.result.playerValue?audio.win():audio.click()});
  ...
  net.on('round_end',m=>{player.setMobileVisible?.(false);setPlayerInput(false);ui.end(m.success,m.balance,m.quota)});net.on('disconnect',()=>ui.toast('Conexão perdida. Recarregue para reconectar.','loss'));
}
